...
let cameraOffset = new THREE.Vector3(0, 0, 1000);

function setCameraTarget(object) {
    targetObject = object;
    // Calculate initial offset
    cameraOffset.copy(camera.position).sub(targetObject.mesh.position);
}
...
let followDistance = 1000; // Distance for the camera to follow the object
...
objects.push(createObject('N1', 3000, 100, 0, 0, 0, 0, 0, 0, '#ff0000', '/planet/1.jpg', true, 100, 0.001));

// Create a satellite orbiting around the first planet
let satellite = createSatellite('Satellite', objects[0], 1000, 1, 50, '#ffffff','/planet/6.jpg');
console.log(satellite);

//objects.push(createObject('N3', 100, 50, 1000, 0, 0, 0, 0, -1.7, '#0000ff', 'planet/4.jpg', true, 1, 0));
...
function createSatellite(name, parentPlanet, orbitRadius, mass, radius, color, textureUrl) {
    let angle = Math.random() * Math.PI * 2;
    let x = parentPlanet.x + orbitRadius * Math.cos(angle);
    let y = parentPlanet.y + orbitRadius * Math.sin(angle);
    let z = parentPlanet.z;
    let speed = Math.sqrt(G * parentPlanet.mass / orbitRadius); // Orbital speed formula
    let vx = -speed * Math.sin(angle);
    let vy = speed * Math.cos(angle);
    let vz = 0;
    let satellite = createObject(name, mass, radius, x, y, z, vx, vy, vz, color, textureUrl, false, 0, 0);
...
function updateOrbit(obj) {
    if (obj.parentPlanet) {
        let parent = obj.parentPlanet;
        let dx = obj.x - parent.x;
        let dy = obj.y - parent.y;
        let dz = obj.z - parent.z;
        let distance = Math.sqrt(dx * dx + dy * dy + dz * dz);
        let speed = Math.sqrt(G * parent.mass / distance);
        let angleXY = Math.atan2(dy, dx);
        let angleZ = Math.atan2(dz, Math.sqrt(dx * dx + dy * dy));
        obj.vx = -speed * Math.sin(angleXY) * Math.cos(angleZ);
        obj.vy = speed * Math.cos(angleXY) * Math.cos(angleZ);
        obj.vz = speed * Math.sin(angleZ);
    }
}
...
function animate() {
    ...
    for (let i = 0; i < objects.length; i++) {
        ...
        if (objects[i].parentPlanet) {
            updateOrbit(objects[i]);
        }
    }
